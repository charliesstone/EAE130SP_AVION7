...
W_furnishings = 217.6*N_crew  # lb, from slide deck, includes seats, ejection seats, etc.
# Wing and engine weights 
W_wing = 2847.0          # lb
W_eng = 3500          # lb

# Placeholder 
W_landing_gear = 2000.0     # lb  <-- replace later
...
W_APU = 300  # lb G230 Honeywell military unit
W_droptank = 3070  # lb
...
# TOGW = (fixed + LG + crew + payload) 

def solve_togw(W_fixed, W_crew, W_payload):
    return (W_fixed + W_crew + W_payload) 

W_fuel_A2A = 13000 # lb, from cruise fuel fraction method, to be replaced with actual fuel fraction method
W_fuel_strike = 10000  # lb, from cruise fuel fraction method, to be replaced with actual fuel fraction method

W_TOGW_A2A = solve_togw(W_fixed, W_crew, W_payload_A2A) + W_fuel_A2A  # lb, from cruise fuel fraction method, to be replaced with actual fuel fraction method
W_TOGW_strike = solve_togw(W_fixed, W_crew, W_payload_strike) + W_fuel_strike  # lb, from cruise fuel fraction method, to be replaced with actual fuel fraction method
...
print(f"Furnishings      = {W_furnishings:.2f} lb")
print(f"Air induction system weight = {W_air_induction:.2f} lb")
print(f"APU              = {W_APU:.2f} lb")
print(f"Droptank         = {W_droptank:.2f} lb")
print()
print(f"fixed weight (before fuel fraction and payload) = {W_fixed:.2f} lb")
