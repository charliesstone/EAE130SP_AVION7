...
W_crew_kg = num_crew * (avg_wt_person + avg_wt_luggage) # kg
W_crew = kgs_to_lbs(W_crew_kg)
print("W_crew: " + str(W_crew_kg) + " kg (" + str(W_crew) + " lbs)")

W_payload_kg = num_passengers * (avg_wt_person + avg_wt_luggage) # kg
W_payload = kgs_to_lbs(W_payload_kg)
print("W_payload: " + str(W_payload_kg) + " kg (" + str(W_payload) + " lbs)")
...
L_D_max = 18
R = 7725            # nmi, from payload-range chart with 35000 kg payload, from the spec data by boeing, found the Range ~ 7725 nmi
E = 30 / 60         # min --> hr
c = 0.52            # lb/(lbf hr)
