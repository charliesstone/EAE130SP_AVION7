...
# ============================================================
# 7g Sustained Turn @ 20kft — T vs S (Constraint-Diagram-consistent)
#
# Uses SAME inputs and SAME maneuver equation as constraint_diag_master.py:
#   TW = (q*Cd0)/(W/S) + (k*n^2/q)*(W/S)
#
# Key: For constraint diagrams, W is treated as a FIXED combat/TO weight
#      for the curve generation (no mission fuel loop, no engine-weight coupling).
# ============================================================
...
S_i = 600.0                # initial wing area assumption (not required, but kept)
...
def TW_7g_from_WS(WS):
    """7g sustained turn constraint (same as maneuver() in your master code)."""
    WS = np.maximum(WS, 1e-9)
    return (q * C_D0) / WS + (k * (n_turn**2) / q) * WS

# ============================================================
# Nested solver (for assignment rubric):
# Inner loop: converge W to fixed target combat weight (W0_target)
...
def inner_loop_weight(W_guess, tol_W=1e-8, max_iter_W=100, relax_W=0.35):
    """
    Converge W to the fixed combat weight W0_target.
    This mimics the 'weight convergence loop' while keeping the diagram-consistent assumption.
    """
...
def outer_loop_thrust_for_S(S, T_guess=20000.0, W_guess=50000.0,
...
# ============================================================
# Run over S grid (this is your required T vs S curve)
# ============================================================
...
plt.plot(S_grid, T_curve, marker='o', linewidth=2, label='7g Sustained Turn @20kft (diagram-consistent)')
...
# 2) Demo convergence at a mid S (grader-proof)
