...
# --- Catapult Takeoff constraint parameters (from your Code 3 structure) ---
...
# ============================================================
# 2) CONSTRAINT FUNCTION: Catapult Takeoff (T/W required)
# ============================================================
def takeoff(rho, Vwod, Vcat, CLMT):
    WS = (0.5 * rho * (Vwod + Vcat)**2 * CLMT)/1.21
    return WS
takeoffWS = takeoff(rho, Vwod, Vcat, CLMT) #wing loading constraint for clearing carrier runway on takeoff

TW_catapult = takeoff(rho, Vwod, Vcat, CLMT)
...
# ============================================================
# 4) OUTER LOOP: converge thrust for Catapult Takeoff constraint at each S
# ============================================================
def solve_outer_loop_for_S_catapult(S, T_guess=30000.0, W0_guess=80000.0,
...
for S in S_grid:
    T_sol, W_sol, conv, itT, Thist, Whist = solve_outer_loop_for_S_catapult(
...
plt.title("Converged T vs S — Catapult Takeoff Constraint")
plt.grid(True)
plt.show()
...
plt.title(f"Outer loop convergence (T) — catapult constraint at S = {S_demo:.0f} ft²")
plt.grid(True)
plt.show()
...
print(f"Catapult Takeoff constraint T/W (constant) = {TW_catapult:.4f}")
