...
n_tar = 7.0
V_BTS = 1000
rho_20k = 0.001267  # slugs/ft^3, density at pressure altitude of 20k ft
...
def calculate_manuever_constraint_coefficient(n_tar, rho, V, C_D_0, AR, e):
    q = 0.5 * rho * V**2
    n = n_tar
    coef_m_1 = q * C_D_0
...
coef_1_maneuver_constraint, coef_2_maneuver_constraint = calculate_manuever_constraint_coefficient(n_tar, rho_20k, V_BTS, C_D_0_cruise, AR, e_cruise)
...
# ===================== MANEUVER STALL COEFFICIENTS (ADDED) =====================
...
rho_SL = rho_20k           # slugs/ft^3 sea level density
Vstall_takeoff = 224     # ft/s  
Vstall_maneuver = np.sqrt(7) * Vstall_takeoff
CLmax_maneuver_stall = C_L_max_cruise  # uses existing C_L_max_takeoff

WS_takeoff_stall = 0.5 * rho_SL * (Vstall_maneuver**2) * CLmax_maneuver_stall
print(f"[TAKEOFF STALL] WS_takeoff_stall = {WS_takeoff_stall:.2f} lb/ft^2")

def TW_from_manuv_at_WS(WS):
    # Cruise constraint: TW = coef1/WS + coef2*WS
    return (coef_1_maneuver_constraint / WS) + (coef_2_maneuver_constraint * WS)

TW_req_manuv_stall = TW_from_manuv_at_WS(WS_takeoff_stall)
print(f"[TAKEOFF STALL] TW required (cruise @ WS_stall) = {TW_req_manuv_stall:.4f}")
# ============================================================================ 
...
coef_1_climb_constraint = calculate_climb_constraint_coefficient(N_eng, k_s, C_L_max_cruise, C_D_0, AR, e, G)
...
TW_maneuver = coef_1_maneuver_constraint/WS + coef_2_maneuver_constraint*WS
...
def calculate_climb_constraint_coefficient(N_eng, k_s, C_L_max, C_D_0, AR, e, G):
    return (1/0.8) * ((k_s**2) / C_L_max * C_D_0 + C_L_max / (np.pi * AR * e * k_s**2) + G)
coef_1_climb_constraint = calculate_climb_constraint_coefficient(N_eng, k_s, C_L_max_takeoff, C_D_0, AR, e, G)
print("Coefficient of takeoff climb:", coef_1_climb_constraint)
...
#plot the resulting T vs S curve from the outer loop convergence for the climb constraint
plt.figure(figsize=(16,9))
