...
#   Engine / fuel  
T_sl_max = 40000.0      # lbf, sea-level max dry thrust
...
ct_max_dry = 0.625      # max dry thrust placeholder from fuel-flow / thrust ratio (assumed based on similar engines)
ct_climb = 0.75         # climb placeholder (assumed based on similar engines, climb typically higher than cruise))
...
#   Mission weights  
W0_strike = 40000     # lb, strike TOGW (iterated back and forth with B1(refined_weight).py,
# moved from 33,000 lb to 36,794 lb to 40,000 lb))

W0_a2a = 38700       # lb, A2A TOGW (iterated back and forth with B1(refined_weight).py
# moved from 30,000 lb to 34,000 lb to 38,700 lb)



#   Max fuel allowed     
fuel_fraction_max = 0.30
...
def march_range_segment(
    ...
    """
    Lecture segmented exponential jet cruise / dash model:
      CL_i = 2 Wi / (rho V^2 S)
      CD_i = CD0 + k CL_i^2
      (L/D)_i = CL_i / CD_i
...
def run_a2a_mission() -> List[SegmentResult]:
    ...
    dash_out = march_range_segment(
        ...
        rho=rho_30k,
        V_kts=2.0 * a_30k_kts,
        ct= 1.0 , #afterburner-like thrust assumption for dash placeholder, higher than strike dash due to A2A loadout and climb-back requirement
